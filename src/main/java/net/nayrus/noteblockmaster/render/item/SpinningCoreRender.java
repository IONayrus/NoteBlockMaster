...
public class SpinningCoreRender extends BaseItemRender{
    ...
    public void renderRotatingRings(PoseStack matrix, MultiBufferSource buffer, Color baseColor, float scale, float alpha, int packedLight){
        ...
        GeometryBuilder.buildTorusWithGradient(matrix.last().pose(), buffer.getBuffer(NBMRenderType.TRANSLUCENT_QUADS), scale, 0.6F, 0.02F, 0, 32, CoreBaseRender.IRON_GRADIENT);
        matrix.popPose();
        matrix.pushPose();
